...
const loadMessagesFromSession = (initial: Message): Message[] => {
  ...
  } catch (error) { console.error("Error reading or parsing chat from sessionStorage:", error); sessionStorage.removeItem(CHAT_SESSION_STORAGE_KEY); }
...
export function useMagisteriumChat({ initialMessage }: UseMagisteriumChatProps) {
  ...
  useEffect(() => {
    try {
      ...
      const shouldSave = messagesToSave.length > 1 || (messagesToSave.length === 1 && messagesToSave[0].id !== initialMessage.id);
      if (shouldSave) {
    ...
    } catch (error) { console.error("[SessionStorage Effect] Error saving chat to sessionStorage:", error); }
  ...
  const sendMessage = async (messageText: string) => {
    ...
    const apiMessagesToSend = [...currentHistory, userMessage]
      .filter(msg => !msg.isLoading && (msg.role === "user" || msg.role === "assistant" || msg.role === "system"))
    ...
    try {
      ...
      if (messageContent && typeof messageContent === 'string') {
        ...
        finalAssistantMessage = {
          ...
          timestamp: new Date( (typeof response.created === 'number' ? response.created * 1000 : undefined) ?? Date.now()),
          citations: citations, 
        ...
        console.error("Received response lacked valid content at expected path (choices[0].message.content). Response:", response);
        finalAssistantMessage = {
           id: placeholderId,
           role: "assistant",
    ...
    setMessages(prev => 
        prev.map(msg => 
            msg.id === placeholderId ? finalAssistantMessage : msg
        )
    );
