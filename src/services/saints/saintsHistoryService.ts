...
async function callMagisteriumProxy(
  ...
  const callable = httpsCallable<CloudFunctionRequestData, MagisteriumProxyResponse>(
    functions,
    'magisteriumProxy',
  );
...
export const getSaintsAndHistory = async (): Promise<SaintsAndHistoryResult> => {
  ...
  const messages: MagisteriumMessage[] = [
    {
      role: 'system',
      content:
        'You are a Catholic historian. Respond ONLY with the JSON data requested, without any introductory text or explanations.',
    },
    {
      role: 'user',
      content: `
  ...
  try {
    ...
    if (!contentJson || typeof contentJson !== 'string') {
      console.error(
        "AI response content is missing, not a string, or in unexpected format within the 'data' field:",
        aiResponse,
      );
    ...
    const finalMessage =
      ...
      error.message.startsWith('AI response content is missing') ||
      error.message.startsWith('Expected JSON data') ||
      error.message.startsWith('Failed to get a valid data payload')
        ? error.message
