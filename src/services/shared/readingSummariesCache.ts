...
export const getCachedSummary = async (
  title: string,
  citation: string,
): Promise<{ summary: string; detailedExplanation?: string } | null> => {
  try {
...
export const saveSummaryToCache = async (
  ...
  if (DEBUG_MODE)
    console.log(`🔍 CACHE DEBUG: Starting saveSummaryToCache for "${title}" (${citation})`);
  ...
  try {
    ...
    const docRef = doc(summariesCollection, readingId);
    if (DEBUG_MODE)
      console.log(`🔍 CACHE DEBUG: Created document reference for collection "${COLLECTION_NAME}"`);

    // Step 2: Clean the summary text
    const cleanedSummary = cleanSummaryText(summary);
    if (DEBUG_MODE)
      console.log(
        `🔍 CACHE DEBUG: Cleaned summary (${cleanedSummary.length} chars): "${cleanedSummary.substring(0, 50)}..."`,
      );
    ...
    if (detailedExplanation) {
      ...
      if (DEBUG_MODE)
        console.log(
          `🔍 CACHE DEBUG: Cleaned detailed explanation (${cleanedExplanation.length} chars)`,
        );
    ...
    if (DEBUG_MODE)
      console.log(`🔍 CACHE DEBUG: About to save document to Firestore:`, {
    ...
    try {
      ...
      if (DEBUG_MODE)
        console.log(
          `✅ CACHE DEBUG: Successfully saved summary to cache: "${title}" (${citation})`,
        );
    } catch (innerError) {
      console.error(`❌ CACHE DEBUG: Error during setDoc operation:`, innerError);
      if (innerError instanceof Error) {
        console.error(
          `CACHE DEBUG: Inner error details - Name: ${innerError.name}, Message: ${innerError.message}`,
        );
    ...
    if (DEBUG_MODE) {
      try {
        ...
        if (savedDoc.exists()) {
          ...
          console.error(
            `❌ CACHE DEBUG: Verification failed! Document does not exist in Firestore after save.`,
          );
        }
      } catch (verifyError) {
...
export const getSummaryWithCache = async (
  title: string,
  citation: string,
  fetchFromApi: (reading: {
    title: string;
    citation: string;
  }) => Promise<{ summary: string; detailedExplanation?: string } | string>,
): Promise<{ summary: string; detailedExplanation?: string }> => {
  if (DEBUG_MODE)
    console.log(`🔍 CACHE DEBUG: Starting getSummaryWithCache for "${title}" (${citation})`);
  const readingId = createReadingId(title, citation);
  ...
  try {
    ...
    if (cachedData) {
      if (DEBUG_MODE)
        console.log(
          `✅ CACHE DEBUG: Cache hit! Using cached summary for: "${title}" (${citation})`,
        );
      return cachedData;
    }
    ...
    if (DEBUG_MODE)
      console.log(
        `🔄 CACHE DEBUG: Cache miss. Fetching summary from API for: "${title}" (${citation})`,
      );
    let apiResult;
    try {
      ...
      if (DEBUG_MODE) {
        if (typeof apiResult === 'string') {
          ...
          console.log(
            `✅ CACHE DEBUG: API returned summary object with summary (${apiResult.summary.length} chars)`,
          );
        }
      }
    } catch (apiError) {
    ...
    if (DEBUG_MODE)
      console.log(`💾 CACHE DEBUG: Saving summary to cache for: "${title}" (${citation})`);
    ...
    try {
      ...
      if (DEBUG_MODE)
        console.log(`✅ CACHE DEBUG: Successfully saved summary to cache synchronously`);
    } catch (cacheError) {
    ...
    const cleanedSummary = cleanSummaryText(summary);
    const cleanedExplanation = detailedExplanation
      ? cleanSummaryText(detailedExplanation)
      : undefined;
...
export const easterVigilSummaries: Record<string, string> = {
  Reading_I_Genesis_1_1_2_2:
    "The Creation account from Genesis describes God creating the heavens and earth in six days and resting on the seventh. This reading emphasizes God as the creator of all things, establishing the foundation of our existence and relationship with Him. It reminds us that all creation is good and that humans are made in God's image, with a special dignity and purpose.",

  Psalm_I_Psalm_104_1_2_5_6_10_12_13_14_24_35:
    "Psalm 104 is a hymn of praise to God as creator and sustainer of all life. It celebrates the wisdom with which God ordered creation and continues to care for it. This psalm reminds us of our dependence on God's providence and invites us to join all creation in praising the Lord.",

  Reading_II_Genesis_22_1_18:
    "The story of Abraham's willingness to sacrifice his son Isaac demonstrates complete faith and obedience to God. This prefigures God's sacrifice of His own Son, Jesus Christ. Abraham's faithfulness is rewarded with God's promise of abundant blessing, foreshadowing the universal salvation that would come through Christ.",
  ...
  Reading_III_Exodus_14_15_15_1:
    "The Exodus account of the Israelites crossing the Red Sea represents God's definitive liberation of His people from slavery in Egypt. This pivotal salvation event prefigures Christian baptism, through which believers pass from the slavery of sin to new life in Christ. The dramatic defeat of Pharaoh's army demonstrates God's power and commitment to saving His people.",

  Psalm_III_Exodus_15_1_2_3_4_5_6_17_18:
  ...
  Reading_IV_Isaiah_54_5_14:
    "Isaiah's prophecy speaks of God as the redeemer who reclaims His people after a period of abandonment. Using the image of marriage, it describes God's eternal love and mercy toward Israel. This reading points to the Church as the new Jerusalem, the bride of Christ, established in justice and peace.",

  Psalm_IV_Psalm_30_2_4_5_6_11_12_13:
    'Psalm 30 is a song of thanksgiving for healing and restoration. The psalmist praises God for bringing him up from the netherworld and turning mourning into dancing. This psalm reflects the Easter joy of moving from death to life and anticipates the resurrection of Christ.',

  Reading_V_Isaiah_55_1_11:
    "Isaiah invites all who thirst to come to the waters, offering God's abundant gifts freely. This reading emphasizes God's generosity and the power of His word, which accomplishes His purpose. It prefigures the sacraments, especially Baptism and Eucharist, and calls us to seek the Lord while he may be found.",

  Psalm_V_Isaiah_12_2_3_4_5_6:
    "This canticle from Isaiah expresses joy in God's salvation. It speaks of drawing water from the springs of salvation, a powerful baptismal image. The song calls all to proclaim God's name and sing His praises, reflecting the Easter joy of the Church.",

  Reading_VI_Baruch_3_9_15_32_4_4:
    "Baruch reflects on Israel's exile as a consequence of abandoning God's wisdom. It praises divine wisdom, which God has revealed to Israel through the Law. This reading highlights the importance of following God's commandments and recognizes that true wisdom comes from God alone.",

  Psalm_VI_Psalm_19_8_9_10_11:
    "Psalm 19 praises God's perfect law that refreshes the soul and gives wisdom to the simple. It celebrates God's commandments as more desirable than gold and sweeter than honey. This psalm reminds us that God's law is not a burden but a gift that brings light and joy.",

  Reading_VII_Ezekiel_36_16_17a_18_28:
    "Ezekiel prophesies God's promise to cleanse His people from their impurities and give them a new heart and spirit. This reading directly prefigures Christian baptism, in which God purifies believers with clean water and places His Spirit within them. It emphasizes God's initiative in salvation and His desire to restore relationship with His people.",

  Psalm_VII_Psalm_42_3_5_43_3_4:
    "Psalms 42-43 express a deep longing for God, comparing it to a deer that longs for flowing streams. The psalmist yearns to appear before God and be led by His light and truth. This reflects the catechumen's desire for baptism and the Christian's ongoing thirst for deeper communion with God.",

  Epistle_Romans_6_3_11:
    'Paul explains that in baptism, Christians are united with Christ in His death and resurrection. This reading is central to the Easter Vigil as it explicitly connects baptism with Easter, teaching that through this sacrament we die to sin and rise to new life in Christ. It calls the baptized to consider themselves dead to sin and alive to God in Christ Jesus.',
...
export const getEasterVigilSummary = (title: string, citation: string): string | null => {
  ...
  if (DEBUG_MODE)
    console.log(
      summary
        ? `✅ Found Easter Vigil summary!`
        : `❌ No Easter Vigil summary found for this reading`,
    );
  return summary || null;
};
  ...
  try {
    ...
    for (const reading of readings) {
      try {
        ...
        console.log(
          `✅ Successfully pre-fetched summary for ${reading.title} (${reading.citation})`,
        );
        successCount++;
      } catch (error) {
        console.error(
          `❌ Error pre-fetching summary for ${reading.title} (${reading.citation}):`,
          error,
        );
        errorCount++;
      }
