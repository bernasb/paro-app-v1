...
export const VoiceProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
      ...
      recognitionInstance.onend = () => {
        ...
        if (isListening) { // If it ended unexpectedly while supposed to be listening
             ...
             if (isWakeWordEnabled && backgroundRecognition) {
                 try { backgroundRecognition.start(); } catch(e) { console.error("Could not restart bg recognition on main end", e); }
             }
        }
      };
    ...
    try {
        ...
        backgroundRecognitionInstance.onresult = (event) => {
            ...
            for (let i = event.resultIndex; i < event.results.length; ++i) {
                ...
                if (segment.isFinal) { // Process final segments for wake word
        ...
        toast({ title: "Wake Word Feature Unavailable", description: "Could not initialize background listening.", variant: "destructive" });
    }
    ...
    return () => {
      ...
      } catch (e) { console.error("Error aborting main recognition:", e); }
      try {
          if (backgroundRecognitionInstance) {
              backgroundRecognitionInstance.abort();
          }
      } catch (e) { console.error("Error aborting background recognition:", e); }
    };
    ...
    if (recognition && !isListening) {
      try {
        ...
        if (backgroundRecognition) {
           try { backgroundRecognition.stop(); } catch(e) { console.warn("Error stopping bg recog before main start", e); }
        }
  ...
  const stopListening = useCallback(() => {
    ...
    if (!isWakeWordEnabled && backgroundRecognition) {
        try { backgroundRecognition.stop(); } catch(e) { console.warn("Error stopping bg recog on manual stop", e); }
    }
      ...
      if (newState) {
        ...
        if (backgroundRecognition) {
           try {
             ...
             toast({ title: "Error", description: "Could not enable wake word listening.", variant: "destructive" });
             return false; // Revert state if start fails
           }
        } else {
             toast({ title: "Error", description: "Background listening service not available.", variant: "destructive" });
        ...
        if (backgroundRecognition) {
           ...
           } catch(e) { console.warn("Error stopping background recognition on toggle", e); }
        }
      }
      return newState;
    });
