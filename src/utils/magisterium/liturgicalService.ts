...
function cleanHtmlContent(htmlString: string, title?: string): string {
  ...
  try {
    ...
    tempDiv.innerHTML = tempDiv.innerHTML.replace(/<div style="text-indent: -3em; margin-left: 3em;">/g, '\n');
      ...
      try {
        ...
        const addReading = (title: string, data: any) => {
          if (data && data.text && typeof data.text === 'string') {
            ...
            readings.push({ title: title, citation: title, content: cleanHtmlContent(data, title) });
          }
        };
