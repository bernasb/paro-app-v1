...
export const proxyMagisteriumRequest = async (
  ...
  console.log(
    'Firebase Auth token retrieved for user:',
    auth.currentUser.email || auth.currentUser.uid,
  );

  // Define the callable function with explicit configuration
  const callMagisteriumProxy = httpsCallable(functions, 'magisteriumProxy', {
    ...
    try {
      ...
      if (
        ...
        if (
          ...
          'choices' in responseData.data &&
          Array.isArray(responseData.data.choices)
        ) {
          ...
          console.error(
            "Invalid nested response structure received from Cloud Function. Expected Magisterium API response format in 'data' field.",
            responseData,
          );
          throw new Error(
            'Received an invalid or unexpected response structure from the AI service proxy.',
          );
        }
      } else {
        // If it doesn't look like a wrapped response, log the error and throw
        console.error(
          'Invalid response structure received from Cloud Function. Expected wrapped response format.',
          responseData,
        );
        throw new Error(
          'Received an invalid or unexpected response structure from the AI service proxy.',
    ...
    choices: [
      {
        message: {
          role: 'assistant',
          content:
            'I apologize, but I am currently experiencing connectivity issues. The server is not responding due to CORS policy restrictions. Please try again later or contact support for assistance.',
        },
      },
    ],
    citations: [],
