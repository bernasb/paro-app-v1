    ...
    while (currentPosition < len) {
        ...
        if (breakPoint === -1) {
            ...
            if (firstSpaceAfterMin !== -1) {
                ...
                let lastSpaceBeforeMin = text.lastIndexOf(' ', minimumEnd - 1);
         ...
         if (breakPoint <= currentPosition) {
             ...
             if (nextSpace !== -1 && nextSpace < currentPosition + maxChars * 1.5) { // Look a bit further
        ...
        let lineText = text.substring(currentPosition, breakPoint).trim();
        if (lineText) { // Avoid adding empty lines
            lines.push(lineText);
        }
...
export const formatPsalmForDisplay = (text: string, minChars = 50, maxChars = 65): string => {
    ...
    segments.forEach(segment => {
        ...
        if (trimmedSegment === responseLine) {
            ...
            if (formattedBlock) { // Only add if there's content
...
export const formatReadingContent = (text: string, minChars = 50, maxChars = 70): string => {
    ...
    return processedSegments.map(segment => segment.replace(/<div style="text-indent: -3em; margin-left: 3em;">/g, '\n')).join('\n\n'); // Or maybe single if semicolon indicates closer connection? Adjust as needed.
};
...
export const formatPsalmContentStructured = (text: string): PsalmLine[] => {
    ...
    if (cleanedOutput.length > 0 && cleanedOutput[0].type === 'blank') { cleanedOutput.shift(); }
    if (cleanedOutput.length > 0 && cleanedOutput[cleanedOutput.length - 1].type === 'blank') { cleanedOutput.pop(); }
...
export const formatGospelAcclamationStructured = (text: string): GospelAcclamationLine[] => {
    ...
    if (mainSentenceEndIndex === -1) { console.warn("Could not reliably determine Acclamation main sentence:", originalText); return [{ type: 'main_sentence', text: originalText }]; }
    ...
    if (verseMatch && verseMatch[1]) { verseText = verseMatch[1].trim(); }
    else if (!originalText.endsWith(mainSentence)) {
        verseText = originalText.substring(mainSentence.length).trim();
        if (verseText.endsWith(mainSentence)) { verseText = verseText.substring(0, verseText.length - mainSentence.length).trim(); }
    } else if (originalText.startsWith(mainSentence) && originalText.endsWith(mainSentence) && originalText !== mainSentence) {
    ...
    if (verseText) {
        ...
        let breakIndex = -1, bestBreakScore = -1;
        // ... (keep the complex break finding logic for semantic split) ...
        for (let i = 1; i < verseText.length - 1; i++) {
            let currentScore = 0;
            if (verseText[i] === ':' || verseText[i] === ';') currentScore = 4; else if (verseText[i] === ',') currentScore = 3; else if (/\s/.test(verseText[i]) && !/\s/.test(verseText[i-1])) currentScore = 1;
            if (currentScore > 0) {
                const proximityScore = midPoint > 0 ? (1 - (Math.abs(i - midPoint) / midPoint)) : 1;
                const totalScore = currentScore + proximityScore;
                if (totalScore > bestBreakScore) { bestBreakScore = totalScore; breakIndex = i + 1; }
            }
        }
        if (breakIndex === -1) {
            for (let i = midPoint; i > 0; i--) { if (/\s/.test(verseText[i])) { breakIndex = i + 1; break; } }
            if (breakIndex === -1) { for (let i = midPoint + 1; i < verseText.length; i++) { if (/\s/.test(verseText[i])) { breakIndex = i + 1; break; } } }
            if (breakIndex === -1) { breakIndex = midPoint > 0 ? midPoint : 1; }
        }
        breakIndex = Math.max(1, breakIndex);
        if (breakIndex >= verseText.length) { breakIndex = verseText.length > 1 ? verseText.length -1 : 1; }
        ...
        else if (!part1Raw && verseText) { // If split failed but verse exists
    ...
    const finalOutput = outputLines.reduce((acc, line, index, arr) => { if (!(line.type === 'blank' && index > 0 && arr[index - 1].type === 'blank')) { acc.push(line); } return acc; }, [] as GospelAcclamationLine[]);
    // Clean leading/trailing blanks
    if (finalOutput.length > 0 && finalOutput[0].type === 'blank') { finalOutput.shift(); }
    if (finalOutput.length > 0 && finalOutput[finalOutput.length - 1].type === 'blank') { finalOutput.pop(); }
    return finalOutput;
};
...
export const formatProseWithQuotes = (text: string): string[] => {
    // ... (keep your original implementation here) ...
     if (!text) return [];
    const sentences = text.replace(/\s+/g, ' ').trim().split(/(?<=[.!?]['"’”]?)\s+/).map(s => s.trim()).filter(s => s);
    ...
    for (const sentence of sentences) {
        ...
        if (hasQuote) {
            if (currentParagraphSentences.length > 0) { paragraphs.push(currentParagraphSentences.join(' ')); currentParagraphSentences = []; }
            paragraphs.push(sentence);
        } else { currentParagraphSentences.push(sentence); }
    }
    if (currentParagraphSentences.length > 0) { paragraphs.push(currentParagraphSentences.join(' ')); }
    return paragraphs;
};
