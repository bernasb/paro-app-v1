...
exports.fetchReadingReferencesMagisterium = fetchReadingReferencesMagisterium;
 ...
 */
async function getLiturgicalDay(date) {
    try {
        ...
        celebrations.sort((a, b) => (b.rank_num || 0) - (a.rank_num || 0));
 ...
 */
async function findNextSpecialDay(startDate, maxDays = 30) {
    ...
    for (let i = 1; i <= maxDays; i++) {
        ...
        const isSpecial = (dayType.startsWith("Sunday") ||
            dayType === "Solemnity" ||
            dayType === "Feast" ||
            dayType === "Feast of the Lord");
        if (isSpecial) {
...
exports.dailyReadingsProxy = (0, https_1.onCall)(async (request) => {
    try {
        ...
        try {
            ...
            throw new https_1.HttpsError('invalid-argument', 'Invalid date format. Please use YYYY-MM-DD.');
        }
        // Get the readings
        ...
        const frontendReadings = (result.readings || []).map((reading) => ({
            title: reading.title,
        ...
        return (0, utils_1.successResponse)(frontendReadings);
    }
    catch (error) {
