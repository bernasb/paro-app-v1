...
exports.dailyReadingsProxy = functions.https.onCall(async (data, context) => {
    // Validate input
    if (!data.date) {
    ...
    try {
        ...
        const response = await axios_1.default.post(GEMINI_API_URL, { date: data.date }, { headers: { 'Authorization': `Bearer ${GOOGLE_API_KEY}` } });
        ...
        return {
            status: 'error',
            responseType: 'error',
            data: error.message || 'Failed to fetch daily readings.'
        };
    }
});
